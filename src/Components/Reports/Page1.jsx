...
const Reports = () => {
  ...
  const handleGenerateReport = (e) => {
    ...
    const newId = `#RPT-00${reportsList.length + 1}`;
    
    // Map campaign and type to Arabic equivalents
    const campaignArMap = {
      'Health Aid': 'المساعدة الصحية',
      'Education Fund': 'صندوق التعليم',
      'Relief Packages': 'حزم الإغاثة'
    };
    ...
    const newReport = {
      id: newId,
      campaign: campaign,
      campaignAr: campaignArMap[campaign] || campaign,
      date: today,
      type: reportType,
      typeAr: typeArMap[reportType] || reportType,
      status: 'Completed',
      statusAr: 'مكتمل'
    };

    setReportsList([newReport, ...reportsList]);
    showToast(isRtl ? 'تم إنشاء التقرير بنجاح وإضافته إلى القائمة' : 'Report generated and added successfully');
  };

  const handleDownloadPDF = () => {
    showToast(isRtl ? 'جاري تنزيل ملف PDF...' : 'Downloading PDF report...');
  };

  const handleExportExcel = () => {
    showToast(isRtl ? 'جاري تصدير ملف Excel...' : 'Exporting Excel spreadsheet...');
  };
